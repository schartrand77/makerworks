...
async function j<T>(p: Promise<Response>) {
  ...
  // let callers decide if they asked for JSON but server returned something else
...
async function getFirst<T>(urls: string[], fallback?: T): Promise<T> {
  for (const u of urls) {
    try { return await get<T>(u) } catch { /* try next */ }
  }
...
function Badge({ tone = 'emerald', children }: { tone?: 'emerald' | 'red' | 'amber' | 'slate'; children: React.ReactNode }) {
  const map: Record<string, string> = {
    emerald: 'bg-emerald-100 text-emerald-900 dark:bg-emerald-500/20 dark:text-emerald-100 border-emerald-500/40',
    red: 'bg-red-100 text-red-900 dark:bg-red-500/20 dark:text-red-100 border-red-500/40',
    amber: 'bg-amber-100 text-amber-900 dark:bg-amber-500/20 dark:text-amber-100 border-amber-500/40',
    slate: 'bg-slate-100 text-slate-900 dark:bg-white/10 dark:text-white/80 border-white/20',
  }
  return <span className={`inline-flex items-center gap-1 rounded-full px-2 h-7 text-xs border ${map[tone]}`}>{children}</span>
}
    ...
    ;(async () => {
      try {
        ...
        const [
          ...
          // Public reads first, then pricing/* (if you add it), then admin/*
          getFirst<PricingSettings>(
            [
              '/api/v1/pricing/settings/latest',
              '/api/v1/admin/pricing/settings/latest',
            ],
            {
              id: 'draft' as UUID,
...
function PrintersCard({
  ...
  const refreshStatus = async (name: string) => {
    try {
      ...
      // swallow; UI shows connected flag anyway
    }
  }
  ...
  const pause = async (name: string) => { await post(`/bambu/bridge/${name}/pause`, {}); onToast(`Paused ${name}`) }
  const resume = async (name: string) => { await post(`/bambu/bridge/${name}/resume`, {}); onToast(`Resumed ${name}`) }
  const stop = async (name: string) => { await post(`/bambu/bridge/${name}/stop`, {}); onToast(`Stopped ${name}`) }

  useEffect(() => { refreshBridge() }, [])
  ...
  return (
    <>
      ...
      >
        ...
        {bridgeErr && <div className="text-sm text-red-600">Bridge error: {bridgeErr}</div>}
        {!bridgeLoading && !bridgeErr && bridge.length === 0 && (
          <div className="text-sm opacity-70">No printers configured. Set <code className="font-mono">BAMBULAB_PRINTERS</code>, <code className="font-mono">BAMBULAB_SERIALS</code>, and <code className="font-mono">BAMBULAB_LAN_KEYS</code> in the environment.</div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {bridge.map((b) => {
            ...
            return (
              <div key={b.name} className="rounded-xl p-3 border border-emerald-500/20 bg-white/60 dark:bg-white/10 backdrop-blur space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className="text-sm font-medium text-emerald-900 dark:text-emerald-100">{b.name}</div>
                    {connected ? <Badge>connected</Badge> : <Badge tone="amber">disconnected</Badge>}
                    {b.last_error ? <Badge tone="red">err</Badge> : null}
                  </div>
                ...
                <div className="text-xs text-emerald-950/70 dark:text-emerald-100/70">
                  <div className="flex flex-wrap gap-3">
                    <span><span className="opacity-60">host:</span> <span className="font-mono">{b.host}</span></span>
                    <span><span className="opacity-60">serial:</span> <span className="font-mono">{b.serial || '—'}</span></span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <LedButton onClick={()=>startPrint(b.name)} disabled={!connected}>Start</LedButton>
                </div>

                <div className="flex flex-wrap gap-2">
                  <LedButton onClick={()=>pause(b.name)} disabled={!connected}>Pause</LedButton>
                  <LedButton onClick={()=>resume(b.name)} disabled={!connected}>Resume</LedButton>
                  <DangerButton onClick={()=>stop(b.name)}>Stop</DangerButton>
                  <button
                    className={ledClasses(true)}
                    onClick={() => setCamOpen(m => ({ ...m, [b.name]: !m[b.name] }))}
                  >
                ...
                {camOpen[b.name] && (
                  <div className="mt-2 rounded-xl overflow-hidden border border-emerald-500/20">
                    {/* MJPEG stream; <img> tag works with multipart/x-mixed-replace */}
                    <img
                      src={`/bambu/bridge/${b.name}/camera`}
                      alt={`${b.name} camera`}
                      className="block w-full h-48 object-cover bg-black/40"
                    />
                  </div>
                )}
                ...
                {stat && (
                  <details className="mt-2">
                    ...
                    <pre className="mt-1 max-h-40 overflow-auto text-xs bg-white/60 dark:bg-white/5 rounded-lg p-2 border border-white/20">
                ...
                {b.last_error && !stat && (
                  <div className="text-xs text-red-700 dark:text-red-300">Last error: {b.last_error}</div>
                )}
              </div>
            )
          })}
