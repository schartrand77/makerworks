...
export default function GlassCard<T extends ElementType = 'div'>({
  ...
  elevation = 'md',
  padding = 'p-6',
  maxWidth = 'none',           // 👈 don’t clamp by default
  glow = true,
  id,
  ...props
}: GlassCardProps<T> & ComponentPropsWithoutRef<T>) {
  const Component = as || 'div'
  const glowClass = glow ? 'shadow-[0_8px_20px_rgba(128,128,128,0.15)]' : ''
  ...
  return (
    <Component
      ...
      className={clsx(
        ...
        'rounded-xl',            // your selection was fine — keep it
